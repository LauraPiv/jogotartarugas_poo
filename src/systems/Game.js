...
export class Game {
    ...
    /**
     * Exibe a animação cinemática de abertura de um pacote de figurinhas.
     * @param {Figurinha} figurinha - Figurinha sorteada.
     * @param {boolean} isNew - Se é uma figurinha inédita.
     * @param {Function} callback - Ação ao fechar a animação.
     */
    exibirAnimacaoPacotinho(figurinha, isNew, callback) {
        ...
        const cardDisplay = document.getElementById('sticker-result-card');
        ...
        pack.classList.remove('pack-open-anim');
        pack.classList.add('pack-shake');
        ...
        // Montar o card de resultado dentro da animação
        cardDisplay.innerHTML = `
            <div class="result-glow"></div>
            ${isNew ? '<div class="new-badge">NOVA!</div>' : ''}
            <div style="width: 200px; height: 200px; margin-bottom: 20px; display: flex; align-items: center; justify-content: center;">
                ${figurinha.imgEmoji.includes('.') 
                    ? `<img src="${figurinha.imgEmoji}" style="width: 100%; height: 100%; object-fit: contain; filter: drop-shadow(0 15px 30px rgba(0,0,0,0.4));">`
                    : `<div style="font-size: 6rem; filter: drop-shadow(0 10px 15px rgba(0,0,0,0.2));">${figurinha.imgEmoji}</div>`
                }
            </div>
            <h2 style="color: #1B4332; font-size: 2.5rem; font-weight: 900; margin: 0;">${figurinha.nome}</h2>
            <div style="background: ${corRaridade}; color: white; padding: 6px 25px; border-radius: 50px; font-weight: 900; font-size: 1.1rem; text-transform: uppercase; letter-spacing: 3px; margin: 15px 0; box-shadow: 0 8px 20px ${corRaridade}66;">
                ${figurinha.raridade}
            </div>
            <p style="color: #444; font-weight: 600; font-size: 1.2rem; line-height: 1.4; max-width: 280px;">${figurinha.descricao}</p>
        `;
        cardDisplay.style.borderColor = corRaridade;

        // Lógica de clique para abrir
        const handleOpen = () => {
            pack.removeEventListener('click', handleOpen);
            pack.classList.remove('pack-shake');
            pack.classList.add('pack-open-anim');

            // Som de abertura ou feedback tátil aqui se necessário
            setTimeout(() => {
        ...
        pack.addEventListener('click', handleOpen);
